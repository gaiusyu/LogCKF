...
import sys
import time
# --- 1. 配置部分 (对应 curl 命令的各个部分) ---
timestamp = int(time.time())
# API 端点 (对应 curl 的 URL)
API_URL = "********"  # <-- 替换成你的真实 URL
...
key_words_list= ["********"]
limit_log_num = 2000
start_time = timestamp-600
end_time = timestamp


PAYLOAD = {
    "psm_list": [
        "bytecloud.openapi.gateway"
    ],
    "keyword_filter": {
        "include": {
    ...
    "start": start_time,
    "end": end_time,
    "timeout_in_ms": 200000
}

# 输出文件的名称
OUTPUT_FILENAME = "Search_response_task.json"
